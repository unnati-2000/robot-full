...
class ArmActionTester(Node):
    ...
    def __init__(self):
        ...
        # Define the target positions based on the servo_controller.yaml values:
        # Each servo seems to have init=500, min=1000, max=0
        # Let's use positions in the middle range (around 500) for safety
        # Try using raw position values directly (0-1000 range)
        self.target_positions = [400, 500, 600, 500, 400]  # Raw servo positions

        # Time (in seconds) to reach the target position from the start
        # Increase to 5 seconds for a slower, safer movement
    ...
    def send_goal(self):
        ...
        # First point - "current" position to start from
        initial_point = JointTrajectoryPoint()
        # For initial point, we'll use positions closer to the middle (500)
        initial_point.positions = [500, 500, 500, 500, 500]  # Initial position
        ...
        # Final point - target position
        final_point = JointTrajectoryPoint()
        final_point.positions = self.target_positions
        ...
        # Set goal tolerance - be more permissive
        for joint in self.joint_names:
            tolerance = FollowJointTrajectory.Goal.GoalTolerance()
            tolerance.name = joint
            tolerance.position = 0.1  # 0.1 unit tolerance
